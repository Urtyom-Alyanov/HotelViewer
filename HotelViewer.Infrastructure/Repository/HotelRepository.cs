...
public class HotelRepository(DataAccess db) : IHostelRepository
{
    private const string Query = "SELECT * FROM Гостиница WHERE ИдентификаторГостиницы = ?";

    private Hostel ConvertToDomain(DataRow row)
    {
        return new Hostel(
    ...
    public Either<RepositoryError, Hostel> GetById(HostelId id)
    {
...
#pragma warning restore CA1416
            ...
            if (table.Rows.Count == 0)
                return Left<RepositoryError, Hostel>(new EntityNotFound(id));

            DataRow row = table.Rows[0];
            
            return Right<RepositoryError, Hostel>(ConvertToDomain(row));
        }).MapLeft(MapToDomainError);
    }

    public Either<RepositoryError, Hostel> Save(Hostel entity)
    {
        return db.ExecuteCommand(Query, command =>
            {
                var table = new DataTable();
